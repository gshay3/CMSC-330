 ...
 * Project: Project 1
 * 
 * This class implements the parser which It performs syntactic analysis
 * on the tokens provided by the Lexer class, following a predefined grammar to construct
 * and render a graphical "scene" composed of various shapes and text elements.
 * The parser also validates the structure of the input program, ensuring that tokens appear in 
 * the correct order and format. I edited the parseImages method to include parsing for ISOSCELES,
 * PARALLELOGRAM, REGULAR_POLYGON, and TEXT.
 */
...
class Parser {
	 ...
	 private void parseImages(Scene scene, Token imageToken) throws LexicalError, SyntaxError, IOException {
	     int height, width, offset, radius, sides; // Added int for sides.
	     ...
	     } else if (imageToken == Token.ISOSCELES) { // Added else if block to handle ISOSCELE.
	         verifyNextToken(Token.HEIGHT);
	         verifyNextToken(Token.NUMBER);
	         height = lexer.getNumber();
	         verifyNextToken(Token.WIDTH);
	     ...
	     } else if (imageToken == Token.PARALLELOGRAM) { // Added else if block to handle PARALLELOGRAM.
	    	 int[] loc = getNumberList(2);
	         Point pointb = new Point(loc[0], loc[1]);
	         verifyNextToken(Token.OFFSET);
	         verifyNextToken(Token.NUMBER);
	         offset = lexer.getNumber();
	         Parallelogram parallelogram = new Parallelogram(color, point, pointb, offset);
	         scene.addImage(parallelogram);
	     } else if (imageToken == Token.REGULAR_POLYGON) { // Added else if block to handle REGULAR_POLYGON.
	     ...
	     } else if (imageToken == Token.TEXT) { // Added else if block to handle TEXT.
